...
public class GraphView extends View implements Runnable{
	...
	private void drawGraph(Canvas canvas){
		...
		paint.setColor(Color.rgb(0, 40, 0));	
		//canvas.drawLine(border, border, border, height - border - bottomspace, paint);
		canvas.drawLine(border, height - border - bottomspace, width, height -border - bottomspace, paint);
		...
		//canvas.drawText(Integer.toString(firstSample), border, height -border/2 - bottomspace, paint);
		//canvas.drawText(Integer.toString(lastSample), width - border/2, height -border/2 - bottomspace, paint);

		paint.setColor(Color.GREEN);
		
		for(int i = 0; i<pointList.size()-1;i++){
			float xa = border+(i*xScale);
			float ya = height - (border+bottomspace+(pointList.get(i)*yScale));
			float xb = border+((i+1)*xScale);
			float yb = height - (border+bottomspace+(pointList.get(i+1)*yScale));
